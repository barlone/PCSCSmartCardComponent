...
begin
  Result := false;
  if not GetResponseFromCard(#$00#$F0#$00#$00, res, sw) then
    exit;
...
begin
  Result := false;
  if not GetResponseFromCard(#$00#$00#$00#$00, res, sw) then
    exit;
...
begin
  Result := false;
  if not GetResponseFromCard(#$00#$10#$00#$00, res, sw) then
    exit;
...
begin
  ...
  if RetVar = SCARD_S_SUCCESS then
    FAttrVendorName := Copy(ABuf, 1, ALen)
  else
    FAttrVendorName := '';
  ...
  if RetVar = SCARD_S_SUCCESS then
    FAttrVendorSerial := Copy(ABuf, 1, ALen)
  else
    FAttrVendorSerial := '';
  ...
  if RetVar = SCARD_S_SUCCESS then begin
    case AIBuf of
      1:
        FAttrICCType := 'ISO7816 Async';
      2:
        FAttrICCType := 'ISO7816 Sync';
    else
...
begin
  ...
  if (Length(res) < 3) or (res[1] <> id[1]) or (res[2] <> id[2]) then
    exit;
...
begin
  ...
  begin
    ...
    begin
      ...
      if FAPDULogging then
        AddLog('<-- ' + Bin2Hex(Result));
    end
    else
    begin
      if FAPDULogging then
        AddLog('<-- error: ' + IntToHex(RetVar, 8));
      Result := '';
      if Assigned(FOnError) then
